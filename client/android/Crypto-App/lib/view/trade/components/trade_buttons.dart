                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                        suffixIcon: Icon(Iconsax.coin_1),
                      ...
                      builder: (context, AsyncSnapshot snapshot) {
                        if (snapshot.hasData) {
                          ...
                          return InkWell(
                            onTap: () {
                              num aa = 0;
                              for (var element in walletViewModel.appTransaction) {
                        ...
                        return InkWell(
                          onTap: () {
                            if (walletViewModel.totalBalance >=
                              ...
                              walletViewModel.appTransaction.add(Transaction(
                                market[index].symbol,
                                amount,
                                (amount * double.parse(market[index].lastPrice)),
                                DateTime.now(),
                              ));
                              ...
                              element.symbol == market[index].symbol)) {
                                ...
                                for (int i = 0; i < walletViewModel.appWallet.length; i++) {
                                  if (market[index].symbol ==
                                      walletViewModel.appWallet[i].symbol) {
                                    walletViewModel.appWallet[i].amount += amount;
                                    walletViewModel.appWallet[i].price += (amount * double.parse(market[index].lastPrice));
                                  }
                                }
                              }

                              num totalBalance = 0;
                              for (var wallet in walletViewModel.appWallet) {
                                totalBalance += wallet.price;
                              }
                              walletViewModel.changeTotalBalance(totalBalance);
